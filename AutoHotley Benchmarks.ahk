...
GetSystemInfo()
{
 Data := RunToVar("cmd /c ""SystemInfo /fo CSV""")
...
RunToVar(CommandLine,WorkingDirectory = 0)
{
 UPtr := A_PtrSize ? "UPtr" : "UInt", WorkingDirectoryType := WorkingDirectory ? "Str" : "Int", StrGetFunc := "StrGet", CMsize := 1, TRead := 0, VarSetCapacity(pa,12,0), NumPut(12,pa), NumPut(1,pa,8), VarSetCapacity(sui,68,0), VarSetCapacity(pi,16,0), VarSetCapacity(CMDout,1,32)
 If DllCall("CreatePipe","UInt*",hRead,"UInt*",hWrite,UPtr,&pa,"Int",0)
 {
  NumPut(68,sui), DllCall("GetStartupInfo",UPtr,&sui), NumPut(257,sui,44), NumPut(hWrite,sui,60), NumPut(hWrite,sui,64)
  If DllCall("CreateProcess","Int",0,UPtr,&CommandLine,"Int",0,"Int",0,"Int",1,"UInt",0,"Int",0,WorkingDirectoryType,WorkingDirectory,UPtr,&sui,"UInt",&pi)
  {
   ProcessPID := NumGet(pi,8)
   Loop
   {
    Sleep, 10
    If Not DllCall("PeekNamedPipe","UInt",hRead,"UInt",0,"UInt",0,"UInt",0,"UInt*",bSize,"UInt",0)
     Break
    Process, Exist, %ProcessPID%
    If Not (ErrorLevel || bSize > 0)
     Break
    If (bSize > 0 && VarSetCapacity(lpBuffer,bSize + 1) && DllCall("ReadFile","UInt",hRead,UPtr,&lpBuffer,"Int",bSize,"UInt*",BytesRead,"Int",0) && BytesRead > 0)
    {
     TRead += BytesRead, VarSetCapacity(CMcpy,BytesRead + CMsize + 1,0), CMcpy := "a", DllCall("RtlMoveMemory",UPtr,&CMcpy,UPtr,&CMDout,"Int",CMsize), DllCall("RtlMoveMemory",UPtr,&CMcpy + CMsize,UPtr,&lpBuffer,"Int",BytesRead), CMsize += BytesRead, VarSetCapacity(CMDout,CMsize + 1,0), CMDout := "a", DllCall("RtlMoveMemory",UPtr,&CMDout,UPtr,&CMcpy,"Int",CMsize), VarSetCapacity(CMDout,-1)
    }
   }
   DllCall("CloseHandle",UPtr,hWrite), DllCall("CloseHandle",UPtr,hRead)
  }
 }
 Return, A_IsUnicode ? %StrGetFunc%(&CMDout,TRead,"CP0") : CMDout
}